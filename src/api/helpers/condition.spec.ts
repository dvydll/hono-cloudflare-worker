...
describe('Condition', () => {
	...
	test('should build simple conditions', () => {
		const sqlParam1 = new SQLParameter({ field: 'field_1', value: 1 });
		...
		expect(condition).toBe('field_1 = $1 field_2 IN ($2, $3)');
	});

	test('should build nested AND conditions', () => {
		const condition = Condition.and(
			new SQLParameter({ field: 'field', operator: 'BETWEEN', value: [1, 2] })
		);
	...
	test('should build deeply nested conditions', () => {
		const condition = Condition.and(
			Condition.group(
				new SQLParameter({ field: 'field_1', value: 1 }),
				Condition.or(new SQLParameter({ field: 'field_2', value: 2, index: 2 }))
			)
		);
		const result = Condition.buildConditions([condition]);
